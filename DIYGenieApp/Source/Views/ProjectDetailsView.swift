...
struct ProjectDetailsView: View {
    ...
    private var orderedPlanSteps: [(originalIndex: Int, step: PlanStep)] {
        planSteps.orderedWithOriginalIndices()
    }
    ...
    private var progressCard: some View {
        let ordered = orderedPlanSteps
        let totalSteps = ordered.count
        let completed = completedSteps
        let completedCount = ordered.filter { completed.contains($0.originalIndex) }.count
        let progress = totalSteps == 0 ? 0 : Double(completedCount) / Double(totalSteps)
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    Text("\(Int(progress * 100))% complete")
                        ...
                        .foregroundColor(.white)
                    Spacer()
                    Text("\(completedCount)/\(max(totalSteps, 1)) steps")
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ForEach(Array(ordered.enumerated()), id: \.offset) { index, entry in
                        let step = entry.step
                        let originalIndex = entry.originalIndex
                        HStack(alignment: .top, spacing: 10) {
                            Image(systemName: completed.contains(originalIndex) ? "checkmark.circle.fill" : "circle")
                                .foregroundColor(completed.contains(originalIndex) ? Color.green : Color.white.opacity(0.7))
    ...
    private func copySteps() {
        ...
        let stepsText = ordered.enumerated().map { index, entry in
            let step = entry.step
